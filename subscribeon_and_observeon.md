...
.subscribe(new Observer<AppInfo>() { [...]
```
`observeOn()`方法将会在指定的调度器上返回结果：如例子中的UI线程。`onBackpressureBuffer()`方法将告诉Observable发射的数据是否比观察者消费的数据要更快，它必须把它们存储在缓存中并提供一个合适的时间给它们。做完这些改变之后，如果我们运行App。就会出现已安装的程序列表：
